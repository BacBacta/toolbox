...
import type { ManifestePython } from '@a237/etabli'
...
const CORPS: Readonly<Record<string, string>> = {
...
async function empreinteDe(texte: string): Promise<string> {
...
let manifeste: ManifestePython
let demandes: string[] = []

/** Un serveur qui rend les corps ci-dessus, et qui note ce qu'on lui demande. */
function serveur(abime: readonly string[] = []): typeof fetch {
  return ((url: string) => {
    ...
    if (corps === undefined) return Promise.resolve(new Response('rien', { status: 404 }))
...
beforeEach(async () => {
  ...
  const fichiers = await Promise.all(
    Object.entries(CORPS).map(async ([nom, corps]) => ({
      nom,
      forme: (nom.endsWith('.wasm') || nom.endsWith('.zip') ? 'octets' : 'texte') as 'octets' | 'texte',
...
describe('poster le moteur au cadre', () => {
  it('sans liste de transférables : sinon le second lancement échoue', async () => {
    const envoi = await moteurPython(manifeste)
    const recus: unknown[][] = []
    const fenetre = { postMessage: (...args: unknown[]) => { recus.push(args) } }

    posterMoteur(fenetre as unknown as Window, envoi)
    posterMoteur(fenetre as unknown as Window, envoi)
