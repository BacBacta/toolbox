...
export type Resultat =
  | { readonly sorte: 'depose'; readonly projet: Projet }
  | { readonly sorte: 'ouvert'; readonly nom: string; readonly fichiers: Projet['fichiers'] }
 ...
 */
export async function deposer(projet: Projet): Promise<Resultat> {
  ...
  if (reponse.ok) return { sorte: 'depose', projet: { ...projet, lien, clef } }
...
export async function recuperer(lien: string): Promise<Resultat> {
  let reponse: Response
  try {
    reponse = await fetch(`/api/p/${lien}`)
  } catch {
  ...
  if (reponse.status === 404) return { sorte: 'introuvable' }
  if (!reponse.ok) return { sorte: 'refuse', pourquoi: 'Ce lien n’a pas pu être ouvert.' }

  const depot = lireDepot(await reponse.json().catch(() => null))
  if (depot === null) return { sorte: 'refuse', pourquoi: 'Ce lien ne contient pas un projet lisible.' }
 ...
 */
export function adressePartagee(lien: string, origine: string): string {
  return `${origine}/?p=${lien}`
}

/** Le lien demandé dans l'adresse, s'il y en a un. */
export function lienDemande(recherche: string): string | null {
