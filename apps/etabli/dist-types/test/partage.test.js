...
function repond(statut, corps) {
    globalThis.fetch = vi.fn(async () => new Response(JSON.stringify(corps), { status: statut }));
}
describe('déposer', () => {
...
describe('un refus qu’on ne connaît pas', () => {
    ...
    it('et même quand le serveur ne dit rien du tout', async () => {
        globalThis.fetch = (async () => new Response('pas du json', { status: 500 }));
