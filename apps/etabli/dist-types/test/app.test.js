...
async function ouvrir() {
    ...
    for (let i = 0; i < 20 && hote.querySelector('.chargement') !== null; i += 1)
        await reposer();
}
function cliquer(selecteur, texte) {
...
describe('un projet ouvert', () => {
    ...
    it('garde ce qu’on écrit, et le rend au retour', async () => {
        ...
        cliquer('.projet');
        expect(hote.querySelector('textarea').value)
...
describe('exporter', () => {
    it('propose un fichier nommé d’après le projet, et qui contient le projet', async () => {
        ...
        HTMLAnchorElement.prototype.click = function remplace() {
