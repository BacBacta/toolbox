 ...
 */
export async function manifestePython() {
    try {
 ...
 */
async function descendreUn(nom, dejaRecus, total, avance) {
    const reponse = await fetch(`${ADRESSE}${nom}`);
    if (!reponse.ok)
        throw new Error(`${nom} : ${reponse.status}`);
 ...
 */
export async function moteurPython(manifeste, avance = () => { }) {
    const textes = {};
    const octets = {};
    const total = manifeste.octets;
    let acquis = 0;
    for (const f of manifeste.fichiers) {
        let brut = await get(clef(manifeste.version, f.nom), MOTEUR);
        ...
        acquis += f.octets;
        if (f.forme === 'texte')
            textes[f.nom] = DECODEUR.decode(brut);
        else
            octets[f.nom] = brut;
    }
    /*
 ...
 */
export function posterMoteur(fenetre, envoi) {
    fenetre.postMessage(envoi, '*');
}
export async function dejaDescendu(manifeste) {
    try {
        for (const f of manifeste.fichiers) {
            const brut = await get(clef(manifeste.version, f.nom), MOTEUR);
            if (brut === undefined || brut.byteLength !== f.octets)
                return false;
        }
        return true;
    }
