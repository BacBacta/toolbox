...
export async function numerosEmis(skeleton: string): Promise<readonly string[]> {
  return (await get<string[]>(skeleton, NUMEROS)) ?? []
}
 ...
 */
export async function numeroter(
  skeleton: string,
  etat: unknown,
  maintenant: Date,
): Promise<unknown> {
  const numero = await reserverNumero(skeleton, maintenant)
  if (numero === null || etat === null || typeof etat !== 'object') return etat
  if (!('numero' in etat)) return etat
  return { ...etat, numero }
}
