...
import type { FormulaireDemande, RenderContext } from '@a237/engine'
...
function poser(etat: unknown, lien?: string): void {
  ...
  act(() => {
    monter(
      <Outil outil={outil} glyphe="✳" ctx={CTX} onChange={() => undefined} onDiffuser={() => undefined} />,
      hote,
    )
  })
}
...
describe('l’aperçu', () => {
  ...
  it('mais ne poste nulle part, et ses champs sont inertes', async () => {
    ...
    expect(hote.querySelector<HTMLInputElement>('.form-champ input')?.disabled).toBe(true)
  })
})
