...
const ESSAI: EtatCompte = { plan: 'essai', credits: 3, expire: null, aUnCode: false }
const DANS_UN_MOIS = Date.now() + 30 * 24 * 60 * 60 * 1000
const ATELIER: EtatCompte = { plan: 'atelier', credits: 40, expire: DANS_UN_MOIS, aUnCode: true }
...
function reponses(...suite: { statut: number; corps: unknown }[]): ReturnType<typeof vi.fn> {
  const appel = vi.fn()
...
function poser(etat: EtatCompte | null): void {
  ...
  act(() => {
    monter(
      <EcranCompte etat={etat} onEtat={(e) => vus.push(e)} onRetour={() => (retours += 1)} />,
      hote,
    )
  })
}
...
function remplir(id: string, valeur: string): void {
  const champ = hote.querySelector<HTMLInputElement>(`#${id}`)
  if (champ === null) throw new Error(`champ introuvable : ${id}`)
  act(() => {
    champ.value = valeur
...
describe('ce qui est mis en avant dépend de ce qui manque', () => {
  const principale = (): string =>
    hote.querySelector('.atelier-option.principale')?.textContent ?? ''
...
describe('l’attente du paiement', () => {
  ...
  it('devient un abonnement quand le paiement aboutit', async () => {
    ...
    try {
      await demarrer()
      reponses(
        { statut: 200, corps: { etat: 'attente', plan: 'essai', credits: 3 } },
        { statut: 200, corps: { etat: 'reussi', plan: 'atelier', credits: 40 } },
        { statut: 200, corps: ATELIER },
      )
      await act(async () => {
