...
beforeEach(() => {
  ...
  act(() => {
    monter(
      <Atelier
        fiches={CATALOGUE}
        onCreer={(skeleton) => ouverts.push(skeleton)}
        onDiscuter={(demande) => discussions.push(demande)}
      />,
      hote,
    )
  })
...
describe('l’étage 1 ne coûte rien', () => {
  ...
  it('et ouvre celui qu’on désigne', () => {
    demander('course scolarite')
    const choix = hote.querySelectorAll<HTMLButtonElement>('.atelier-option')
    act(() => choix[0]?.click())
  ...
  it('propose des exemples tant qu’on n’a rien tapé, et les reprend au clic', () => {
    ...
    const exemples = hote.querySelectorAll<HTMLButtonElement>('.atelier-exemple')
