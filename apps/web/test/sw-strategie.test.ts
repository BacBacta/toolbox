...
function requete(p: Partial<Parameters<typeof strategiePour>[0]> = {}) {
  return {
    methode: 'GET',
    mode: 'no-cors',
...
describe('strategiePour', () => {
  ...
  it('laisse passer une page publiée : elle n’appartient pas à l’application', () => {
    ...
    expect(strategiePour(
      requete({ mode: 'navigate', destination: 'document', url: `${ORIGINE}/d/K7M2XQ4BN9PZ` }),
      ORIGINE,
    )).toBe('reseau')
  })
  ...
  it('ne confond pas un chemin de l’application avec un chemin du serveur', () => {
    ...
    expect(strategiePour(
      requete({ mode: 'navigate', destination: 'document', url: `${ORIGINE}/depart` }),
      ORIGINE,
    )).toBe('coquille')
  })
