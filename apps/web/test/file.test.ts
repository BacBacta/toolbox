...
import {
  creerOutil, enregistrerOutil, filerPublication, lireFile, lireOutil, listerOutils,
  noterPublication, nouvelIdentifiant, retirerDeLaFile, supprimerOutil,
...
function horsLigne(): ReturnType<typeof vi.fn> {
  const appel = vi.fn().mockRejectedValue(new TypeError('hors ligne'))
  globalThis.fetch = appel as unknown as typeof fetch
  return appel
}

async function enFile(skeleton = 'devis'): Promise<string> {
