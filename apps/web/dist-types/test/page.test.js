...
function poser(etat, onChange = vi.fn(), onDiffuser = vi.fn()) {
    const outil = {
        id: 'abc', skeleton: 'compose-page', nom: 'Essai', etat, version: 0,
        creeLe: LE_9_SEPT.getTime(), majLe: LE_9_SEPT.getTime(),
    };
    act(() => {
        monter(_jsx(Outil, { outil: outil, glyphe: "\u2733", ctx: CTX, onChange: onChange, onDiffuser: onDiffuser }), hote);
    });
...
describe('l’aperçu', () => {
    ...
    it('porte le bouton WhatsApp que le lecteur touchera', () => {
        poser(PAGE);
        const lien = hote.querySelector('a.vitrine-appel');
...
describe('la modification', () => {
    it('édite la configuration elle-même : ce qu’on change est ce qui se publie', () => {
        ...
        cliquer('Modifier');
        const champ = [...hote.querySelectorAll('input')].find((i) => i.value === 'Quincaillerie Bépanda');
        if (champ === undefined)
            throw new Error('le nom ne s’édite pas');
        act(() => {
...
describe('le partage', () => {
    it('emporte le lien, les prix et le numéro', () => {
        ...
        const batir = onDiffuser.mock.calls[0]?.[0];
