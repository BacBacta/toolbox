...
function flux(evenements, taille = 11) {
 ...
 */
function repond(evenements, statut = 200) {
    const appel = vi.fn().mockImplementation(() => Promise.resolve({
...
function poser(demande = 'un suivi de mes livraisons de gaz') {
    act(() => {
        monter(_jsx(EcranAgent, { demande: demande, onCreer: (skeleton, _e, compose, fcfa) => creations.push({ skeleton, compose, fcfa }), onFermer: () => {
                ferme++;
            } }), hote);
    });
}
...
describe('le deuxième tour', () => {
    it('renvoie l’outil et le laissez-passer : il ne repaie pas un crédit', async () => {
        ...
        const champ = hote.querySelector('.agent-saisie input');
        if (champ === null)
        ...
        const corps = JSON.parse((appel.mock.calls[1]?.[1]).body);
...
describe('la fenêtre montre la chose elle-même quand elle se dessine seule', () => {
    ...
    it('un formulaire aussi, mais inerte : il ne poste nulle part', async () => {
        ...
        expect(hote.querySelector('.fenetre-vue .form-champ input')?.disabled).toBe(true);
    });
