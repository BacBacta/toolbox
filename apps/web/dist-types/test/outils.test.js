...
describe('le registre des outils', () => {
    it('couvre les squelettes qui ont un écran, et le dit', () => {
        expect(Object.keys(CHARGEURS).sort()).toEqual([
            ...
            'compose-calcul', 'compose-formulaire', 'compose-page', 'course', 'cv', 'dette', 'devis', 'facture', 'motivation',
...
describe('le curriculum vitæ', () => {
    ...
    it('s’ouvre sur la feuille dès qu’il porte un nom', async () => {
        const module = await CHARGEURS.cv();
        const neuf = module.creer('cv', LE_9_SEPT, EXTRAIT_VIDE);
        const nomme = { ...neuf.etat, identite: { ...cv.defaults.identite, nom: 'Adèle' } };
    ...
    it('change de gabarit sans rien faire ressaisir', async () => {
        ...
        act(() => {
            monter(_jsx(module.Outil, { outil: outil('cv', etat), glyphe: "\u25EB", ctx: CTX, onChange: change, onDiffuser: vi.fn() }), hote);
        });
        const editorial = [...hote.querySelectorAll('.cv-gabarit')].find((b) => b.textContent?.includes('Éditorial') === true);
        act(() => editorial?.click());
