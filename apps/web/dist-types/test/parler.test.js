...
function repond(statut, corps, texte) {
...
const DIS = { messages: [{ qui: 'personne', texte: 'un suivi' }] };
async function signes() {
    const vus = [];
    for await (const s of parler(DIS))
        vus.push(s);
    return vus;
}
...
describe('le flux', () => {
    it('rend les événements dans l’ordre', async () => {
        repond(200, {}, 'data: {"sorte":"ebauche","ebauche":{"mot":"Je te","famille":null,"titre":"","pieces":[]}}\n\n' +
