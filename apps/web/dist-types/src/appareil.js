...
const CLEF = 'jeton';
let enMemoire = null;
export async function jetonDeCetAppareil() {
    if (enMemoire !== null)
        return enMemoire;
    const range = await get(CLEF, APPAREIL);
...
export async function entetesDAppareil() {
 ...
 */
export async function entetesSiPossible() {
    try {
...
export function oublierEnMemoire() {
    enMemoire = null;
}
