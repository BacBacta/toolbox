...
export function Outil(props) {
    ...
    return (_jsx(CadreDocument, { titre: props.outil.nom, glyphe: props.glyphe, sousTitre: `Facture N° ${etat.numero}`, onglet: onglet, onOnglet: setOnglet, onDiffuser: () => props.onDiffuser(facture.share(etat, props.ctx)), children: onglet === 'Document' ? (_jsxs(_Fragment, { children: [_jsx(Manquements, { manquements: controleLegal(etat), consequence: "Sans elles, un client qui veut d\u00E9duire ne pourra pas s\u2019en servir.", onCompleter: () => setOnglet('Modifier') }), _jsx(DocumentFacture, { etat: etat, maintenant: props.ctx.maintenant })] })) : (_jsx(ChampsSchema, { schema: facture.schema, valeur: etat, masques: MASQUES, onChange: props.onChange })) }));
}
