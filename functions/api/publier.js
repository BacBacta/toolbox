...
*/
var NON_PUBLIABLES = ["ardoise", "callbox"];
function publiable(skeleton) {
	return !NON_PUBLIABLES.includes(skeleton);
}
/** Pourquoi un outil ne se publie pas, dit à celui qui essaie. */
function pourquoiNonPubliable(skeleton) {
	if (skeleton === "ardoise") return "Une ardoise ne se publie pas : elle porte des noms et des dettes. Sa carte est pour toi, et les relances partent une par une.";
	if (skeleton === "callbox") return "La recette du jour ne se publie pas. Garde la carte pour toi ou pour ton patron.";
	return null;
}
/**
...
var ID_COMPOSE_PAGE = "compose-page";
var schemaPage = {
	type: "object",
	additionalProperties: false,
	required: [
...
sansCommentaires(lecture_default);
sansCommentaires(carte_default);
sansCommentaires(a4_default) + sansCommentaires(a4_default$1);
sansCommentaires(vitrine_default);
/**
...
*/
function controler(recu, detenu) {
	const d = recu;
	if (!lienValide(typeof d?.lien === "string" ? d.lien : "")) return refus(400, "lien-invalide");
	...
	if (!squeletteConnu(skeleton) && !COMPOSES.has(skeleton)) return refus(400, "squelette-inconnu");
...
async function onRequest(contexte) {
	...
	if (Number.isFinite(annonce) && annonce > 262144) return json(413, { erreur: "instantane-trop-gros" });
	let recu;
	try {
	...
	const depot = recu;
	const detenu = await contexte.env.INSTANTANES.get(depot?.lien ?? "", "json");
	...
	return json(200, {
		lien: depot.lien,
		version: depot.instantane.version
	});
}
//#endregion
export { onRequest };
